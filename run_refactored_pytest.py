...
REFACTORED_ROOT_PATH = Path("refactored_results")
...
def save_results(result_dir: Path, test_result: dict[str, object], status: str) -> None:
    """Speichert die Ergebnisse einer Iteration."""
    result_dir.mkdir(parents=True, exist_ok=True)
    write_text_file(result_dir / "pytest_stdout.txt", str(test_result.get("stdout", "")))
    write_text_file(result_dir / "pytest_stderr.txt", str(test_result.get("stderr", "")))
    write_text_file(
        result_dir / "pytest_returncode.txt",
        str(test_result.get("returncode", "")),
    )
    write_text_file(result_dir / "status.txt", status)
...
def process_iteration(
    ...
    if not code_dir.exists():
        save_results(result_dir, {"stdout": "", "stderr": "", "returncode": -1}, "FAILURE")
        write_text_file(result_dir / "note.txt", f"Code-Verzeichnis fehlt: {code_dir}")
        return

    snapshot_files = collect_snapshot_files(code_dir)
    if not snapshot_files:
        save_results(result_dir, {"stdout": "", "stderr": "", "returncode": -1}, "FAILURE")
        write_text_file(result_dir / "note.txt", f"Keine Python-Dateien in {code_dir}")
        return
    ...
    status = "SUCCESS" if test_result.get("success") else "FAILURE"
    save_results(result_dir, test_result, status)
    write_text_file(result_dir / "timestamp.txt", datetime.now().isoformat())
...
def main() -> None:
    ...
    if not iteration_dirs:
        note_path = ensure_within_root(results_root, results_root / "note.txt")
        write_text_file(note_path, f"Keine iteration_XX Ordner gefunden in {refactored_root}")
        return

    for iteration_dir in iteration_dirs:
        process_iteration(iteration_dir, project_src, results_root, backup_dir)
